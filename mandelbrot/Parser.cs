...
namespace info.lundin.math
{
    ...
    {
        public static string c_sharp_string = "";
        ...
        private bool
        isDefaultVariables(string exp)
        {
            if (exp == "z" || exp == "c" || exp == "r" || exp == "p" || exp == "n")
                return true;
            return false;
        }
        ...
        toValue(Node tree)
        {
            ...
            {
                if (htbl.ContainsKey(tree.getValue()))
                {
                    return (string)htbl[tree.getValue()];
                }
            ...
            else if (tree.getType() == Node.TYPE_VARIABLE)
            {
                ...
                {
                    return "System.Math." + tmp;
                    //return ((double)spconst[tmp]);
                }

                // normal variable, get value
                if (htbl.ContainsKey(tmp))
                {
                        return  (string)htbl[tmp];
                    
                }
                if (isDefaultVariables(tmp))
                {
                    return tmp ;
                }
            ...
            if (tree.arguments() == 2)
            {
                ...
                if (op.Equals("+"))
                    return string.Format("({0}+{1})", toValue(arg1), toValue(arg2));
                else if (op.Equals("-"))
                    return string.Format("({0}-{1})", toValue(arg1), toValue(arg2));
                else if (op.Equals("*"))
                    return string.Format("({0}*{1})", toValue(arg1), toValue(arg2));
                    ...
                    return string.Format ( "(Complex.Pow( {0},{1}))" ,toValue(arg1), toValue(arg2) );
                else if (op.Equals("log"))
                    return string.Format("(Complex.Log( {0},{1}))", toValue(arg1), toValue(arg2));
            ...
            {
                if (op.Equals("sqrt"))
                    return string.Format ( "(Complex.Pow({0}, 0.5))", toValue(arg1));
                else if (op.Equals("sin"))
                    return string.Format ( "(Complex.Sin({0}))", toValue(arg1));
                else if (op.Equals("cos"))
                    return string.Format("(Complex.Cos({0}))", toValue(arg1));
                else if (op.Equals("tan"))
                    return string.Format ( "(Complex.Tan({0}))", toValue(arg1));
                else if (op.Equals("asin"))
                    return string.Format("(Complex.Asin({0}))", toValue(arg1));
                else if (op.Equals("acos"))
                    return string.Format("(Complex.Acos({0}))", toValue(arg1));
                else if (op.Equals("atan"))
                    return string.Format ( "(Complex.Atan({0}))", toValue(arg1));
                else if (op.Equals("ln"))
                    return string.Format("(Complex.Log({0}))", toValue(arg1));
                else if (op.Equals("exp"))
                    return string.Format("(Complex.Exp({0}))", toValue(arg1));
                else if (op.Equals("cotan"))
                    return string.Format ( "(1/Complex.Tan({0}))" ,toValue(arg1));
                else if (op.Equals("acotan"))
                    return string.Format ( "(Math.PI / 2 - Complex.Atan({0}))", toValue(arg1));
                else if (op.Equals("abs"))
                    return string.Format ( "(Math.Abs({0}).Magnitude )", toValue(arg1));
                
                else if (op.Equals("sinh"))
                {
                    return string.Format ( "(Complex.Sinh({0}))", toValue(arg1));
                }
                else if (op.Equals("cosh"))
                {
                    return string.Format("(Complex.Cosh({0}))", toValue(arg1));
                }
                else if (op.Equals("tanh"))
                {
                    return string.Format ( "(Complex.Tanh({0}))", toValue(arg1));
                }
    ...
    public class Node
    {
        ...
        //private Complex value = Double.NaN;
    ...
    public class Program
    {
        public static void ___Main(string[] args)
        {
            ExpressionParser parser = new ExpressionParser();
             			...
             			string result = parser.Parse( "x/cos(y)", h );
